...
st.set_page_config(page_title="InsureRisk | Insurance Risk Segmentation", layout="wide", page_icon="\U0001f3e6")
with st.sidebar:
    st.header("\u2699 Config"); n = st.slider("Samples", 2000, 20000, 10000, 1000)
    tau = st.slider("Threshold", 0.05, 0.95, 0.50, 0.05)
    st.caption("Solvency II | NAIC AI Bulletin")
...
with t4:
    ...
    st.caption("These model families are the production stack for Solvency II Pillar-1 capital calculation, NAIC AI-bulletin model governance, and commercial ratemaking: GLM for rate filings, GBM for signal, copulas for aggregate loss dependence.")
